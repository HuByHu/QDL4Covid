...
from models.Blocks import DiscriminatorTop, DiscriminatorBlock, InputBlock, GSynthesisBlock, QDiscriminatorBlock, QDiscriminatorTop
from models.CustomLayers import EqualizedConv2d, PixelNormLayer, EqualizedLinear, Truncation
from models.CustomLayers import QEqualizedConv0, QPool, QBlur, QEqualizedConvDown, QPoolDown, QEqualizedConvLast, QPoolLast, QDense, encoding
from data.rna_process import get_rna_model_number, get_ori_sp
...
class Discriminator(nn.Module):
    ...
    def __init__(self, resolution, num_channels=1, fmap_base=8192, fmap_decay=1.0, fmap_max=512,
        ...
        # Returns the number of feature maps in the first stage layer. If stage<=4, 
        # the number of feature maps is 512, and if stage>4, 
        # the number of feature maps is halved for each additional layer.
        def nf(stage):
            return min(int(fmap_base / (2.0 ** (stage * fmap_decay))), fmap_max)
...
class QDiscriminator(nn.Module):
    ...
    def __init__(self, resolution, num_channels=1, fmap_base=8192, fmap_decay=1.0, fmap_max=512,
        ...
        # 返回在第stage层中特征图数量，stage<=4，特征图数量为512，stage>4，每多一层特征图数量减半。
        #  Returns the number of feature maps in the first stage layer.
        #  If stage<=4, the number of feature maps is 512, and 
        # if stage>4, the number of feature maps is halved for each additional layer.
        def nf(stage):
            return min(int(fmap_base / (2.0 ** (stage * fmap_decay))), fmap_max)
        ...
        for res in range(resolution_log2, 2, -2):
            # name = '{s}x{s}'.format(s=2 ** res)
            blocks.append(QDiscriminatorBlock(nf(res - 1), nf(res - 3), res,
                                             gain=gain, use_wscale=use_wscale))
        self.blocks = nn.ModuleList(blocks)

        # Building the final block.
        self.final_block = QDiscriminatorTop(in_channels=nf(3), intermediate_channels=nf(3),
                                            gain=gain, use_wscale=use_wscale)
...
class StyleGAN:
    ...
    def __init__(self, structure, resolution, num_channels, latent_size,
        ...
        self.dis = QDiscriminator(num_channels=num_channels,
                                  resolution=resolution,
                                  structure=self.structure,
                                  **d_args).to(self.device)
        
        self.dis1 = Discriminator(num_channels=num_channels,
                                  resolution=resolution,
                                  structure=self.structure,
                                  **d_args).to(self.device)
