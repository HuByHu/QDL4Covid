...
from models.CustomLayers import QuEqualizedConv0, QuPool, QuBlur, QuEqualizedConvDown, QuPoolDown, QuEqualizedConvLast, QuPoolLast, QuDense
...
class QuDiscriminatorBlock(nn.Sequential):
    def __init__(self, n_qubit, gain, use_wscale):
        super().__init__(OrderedDict([
            ('qconv0', QuEqualizedConv0(n_qubit, gain=gain, use_wscale=use_wscale)),
            # out channels nf(res-1)
            ('qpool', QuPool(n_qubit, gain=gain, use_wscale=use_wscale)),
            ('qblur', QuBlur(n_qubit, gain=gain, use_wscale=use_wscale)),
            ('qconv_down', QuEqualizedConvDown(n_qubit, gain=gain, use_wscale=use_wscale)),
            ('qpool_down', QuPoolDown(n_qubit, gain=gain, use_wscale=use_wscale))]))


class QuDiscriminatorTop(nn.Sequential):
    def __init__(self, gain, use_wscale):

        layers = []

        layers.append(('qconv_last', QuEqualizedConvLast(gain=gain, use_wscale=use_wscale)))
        layers.append(('qpool_last', QuPoolLast(gain=gain, use_wscale=use_wscale)))
        layers.append(('qdense', QuDense(gain=gain, use_wscale=use_wscale)))
