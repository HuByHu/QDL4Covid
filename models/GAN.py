...
from models.Blocks import DiscriminatorTop, DiscriminatorBlock, InputBlock, GSynthesisBlock, QuDiscriminatorBlock, QuDiscriminatorTop
from models.CustomLayers import EqualizedConv2d, PixelNormLayer, EqualizedLinear, Truncation
from models.CustomLayers import QuEqualizedConv0, QuPool, QuBlur, QuEqualizedConvDown, QuPoolDown, QuEqualizedConvLast, QuPoolLast, QuDense, encoding
from data.rna_process import get_rna_model_number, get_ori_sp, get_ori_sp_new
...
class GSynthesis(nn.Module):
    ...
    def __init__(self, dlatent_size=512, num_channels=1, resolution=1024,
        ...
        self.depth = int(resolution_log2 / 2)

        self.num_layers = 2 * self.depth
        ...
        blocks = []
        for res in range(4, resolution_log2 + 1, 2):
            last_channels = nf(res - 3)
            channels = nf(res - 1)
        ...
        self.temporaryUpsampler = lambda x: interpolate(x, scale_factor=4)
...
class Generator(nn.Module):
    ...
    def __init__(self, resolution, latent_size=512, dlatent_size=512,
        ...
        # Setup components.
        self.num_layers = int((int(np.log2(resolution)) / 2) * 2)
    ...
    def forward(self, latents_in, depth, alpha, labels_in=None):
        ...
        fi_min = fake_images.min()
        fi_max = fake_images.max()
        fake_images = (fake_images-fi_min)/(fi_max-fi_min) ##[0,1]
        # print('fake_images:', fake_images)
        # fake_images = (2*fake_images-fi_min-fi_max)/(fi_max-fi_min) #[-1,1]        
...
class Discriminator(nn.Module):
    ...
    def __init__(self, resolution, num_channels=1, fmap_base=8192, fmap_decay=1.0, fmap_max=512,
        ...
        :param fmap_base: Overall multiplier for the number of feature maps.(18-2)*512.
        :param fmap_decay: log2 feature map reduction when doubling the resolution.
        :param fmap_max: Maximum number of feature maps in any layer.
        :param nonlinearity: Activation function: 'relu', 'lrelu'
        :param use_wscale: Enable equalized learning rate? He initializetion.
        ...
        :param blur_filter: Low-pass filter to apply when resampling activations. None = no filtering.[1,2,1]
        :param structure: 'fixed' = no progressive growing, 'linear' = human-readable
        :param kwargs: Ignore unrecognized keyword args.
        """
        super(Discriminator, self).__init__()

        # 返回在第stage层中特征图数量，stage<=4，特征图数量为512，stage>4，每多一层特征图数量减半。
        def nf(stage):
            return min(int(fmap_base / (2.0 ** (stage * fmap_decay))), fmap_max)
        ...
        blocks = []
        from_rgb = []
        for res in range(resolution_log2, 2, -2):
            # name = '{s}x{s}'.format(s=2 ** res)
            blocks.append(DiscriminatorBlock(nf(res - 1), nf(res - 3),
        ...
        self.final_block = DiscriminatorTop(self.mbstd_group_size, self.mbstd_num_features,
                                            in_channels=nf(1), intermediate_channels=nf(1),
                                            gain=gain, use_wscale=use_wscale, activation_layer=act)
        from_rgb.append(EqualizedConv2d(num_channels, nf(1), kernel_size=1,
                                        gain=gain, use_wscale=use_wscale))
        self.from_rgb = nn.ModuleList(from_rgb)

        # register the temporary downSampler
        self.temporaryDownsampler = nn.AvgPool2d(4)
...
class QuDiscriminator(nn.Module):
    
    def __init__(self, resolution, num_channels=1, fmap_base=8192, fmap_decay=1.0, fmap_max=512,
                 use_wscale=True, structure='linear', **kwargs):
        """
        Quantum version discriminator used in the StyleGAN paper.
        """
        super(QuDiscriminator, self).__init__()
        ...
        for res in range(resolution_log2, 2, -2):
            # name = '{s}x{s}'.format(s=2 ** res)
            blocks.append(QuDiscriminatorBlock(res, gain=gain, use_wscale=use_wscale))
        self.blocks = nn.ModuleList(blocks)

        # Building the final block.
        self.final_block = QuDiscriminatorTop(gain=gain, use_wscale=use_wscale)
        
        self.temporaryDownsampler = nn.AvgPool2d(4)

    def forward(self, images_in, depth, alpha=1., labels_in=None):
        """
        images_in:[1,1,2 ** res,2 ** res] or [2 ** res,2 ** res]经典数据
        """
        ...
        # resnet
            ...
            if depth > 0:
                residual = encoding((self.temporaryDownsampler(images_in.unsqueeze(0))).squeeze())
                # residual = (self.temporaryDownsampler(images_in.unsqueeze(0))).squeeze()
                straight = self.blocks[self.depth - depth - 1](encoding(images_in))
                # straight = self.blocks[self.depth - depth - 1](images_in)
                x = (alpha * straight) + ((1 - alpha) * residual)
...
class StyleGAN:
    ...
    def __init__(self, structure, resolution, num_channels, latent_size,
        ...
        self.structure = structure
        self.depth = int(int(np.log2(resolution)) / 2)
        self.latent_size = latent_size
        self.device = device
        ...
        self.dis = QuDiscriminator(num_channels=num_channels,
                                  resolution=resolution,
                                  structure=self.structure,
    ...
    def __progressive_down_sampling(self, real_batch, depth, alpha):
        ...
        down_sample_factor = int(np.power(2, 2 * (self.depth - depth - 1)))
        prior_down_sample_factor = max(int(np.power(2, 2 * (self.depth - depth))), 0)

        ds_real_samples = AvgPool2d(down_sample_factor)(real_batch)

        if depth > 0:
            prior_ds_real_samples = interpolate(AvgPool2d(prior_down_sample_factor)(real_batch), scale_factor=4)
        else:
    ...
    def optimize_discriminator(self, noise, real_batch, depth, alpha):
        ...
        for _ in range(self.d_repeats):
            ...
            # print('rs',real_samples.size())
            # print('fs',fake_samples.size())
    ...
    def train(self, dataset, num_workers, epochs, batch_sizes, fade_in_percentage, logger, output,
              num_samples=1, start_depth=0, feedback_factor=100, checkpoint_factor=1):
        """
        ...
        for current_depth in range(start_depth, self.depth):
            current_res = np.power(2, (current_depth + 1) * 2)
            ...
            for epoch in range(1, epochs[current_depth] + 1):
                ...
                for (i, batch) in enumerate(data, 1):
                    ...
                    # print('inputsize',gan_input.size())
                    ...
                    # provide a loss feedback
                        if i % int(total_batches / feedback_factor + 1) == 0 or i == 1:
                            ...
                            logger.info("Elapsed: [%s] Step: %d  Batch: %d  D_Loss: %f  G_Loss: %f"
                        ...
                        # create a grid of samples and save it
                            ...
                            gen_img_file = os.path.join(output, 'samples', "gen_" + str(current_depth)
                                                    + "_" + str(epoch) + "_" + str(i) + ".txt")
                            gen_pos_file = os.path.join(output, 'samples', "gen_pos_" + str(current_depth)
                                                    + "_" + str(epoch) + "_" + str(i) + ".txt")
                            gen_true_file = os.path.join(output, 'samples', "gen_true_" + str(current_depth)
                                                    + "_" + str(epoch) + "_" + str(i) + ".txt")
                            gen_diag_file = os.path.join(output, 'samples', "gen_diag_" + str(current_depth)
                                                    + "_" + str(epoch) + "_" + str(i) + ".txt")

                            with torch.no_grad():
                                chg_pos, ori_sp = get_ori_sp_new()

                                similar_rate = 0.97
                                fix_number = math.floor(current_res*similar_rate)
                                ...
                                samples_diag = torch.diag(samples)
                                samples_diag_true = torch.sqrt(samples_diag)
                                np.savetxt(gen_diag_file, samples_diag_true.cpu(), delimiter = ',')
                        
                                samples_val = samples_diag_true.sort().values
                                samples_fix = samples_diag_true.sort().indices[0:fix_number]
                                samples_chg = samples_diag_true.sort().indices[fix_number:]
                                samples_code = ''
                                samples_code_tensor = torch.zeros(len(samples_val))
                                for i in range(len(samples_val)):
                                    if i in samples_fix:
                                        samples_code += '0'
                                    elif i in samples_chg:
                                        samples_code += '1'
                                        samples_code_tensor[i] = 1
                                f = open(gen_img_file, "w", encoding="utf8")
                                f.write(samples_code)
                                f.close
                                np.savetxt(gen_pos_file, samples_chg, delimiter = ',')
                                
                                for i in range(len(samples_code_tensor)):
                                    ori_sp[chg_pos[i]] = samples_code_tensor[i]
                                np.savetxt(gen_true_file, ori_sp, delimiter = ',')
            ...
            os.makedirs(os.path.join(output, 'samples'), exist_ok=True)
            dloss_file = os.path.join(output, 'samples',  "dloss.txt")
            gloss_file = os.path.join(output, 'samples',  "gloss.txt")
            np.savetxt(dloss_file, torch.tensor(dloss), delimiter = ',')
            np.savetxt(gloss_file, torch.tensor(gloss), delimiter = ',')
